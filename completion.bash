_ssh_shorty_complete() {
    ...
    local subcommands="--list --add --set --set-mac --rename --remove --tag --untag --sync --ping --poll --edit --paths --help --update --fav --status --watch --run --run-script --sysinfo --tail --tunnel --close --register --export-ssh-config --keydeploy --last --import -u --upload -d --download --view -m"
    ...
    if [[ "$cword" -eq 1 ]]; then
        ...
        case "$first" in
            --set|-s|--set-mac|--rename|--remove|-r|--poll|--tunnel|-t)
                [[ "$cword" -eq 2 ]] && \
                    COMPREPLY=( $(compgen -W "${machines[*]}" -- "$cur") )
                ;;
            --ping|-p)
