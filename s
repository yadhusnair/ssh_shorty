...
_lookup_target() {
    awk -v n="$1" '$1 == n {print $2; exit}' "$MAPFILE"
}
...
# Open multiple devices each in its own tmux window
_tmux_multi() {
    local -a nicks=("$@")
...
case "$1" in
    ...
    --poll)
        ...
        if [[ -z "$TARGET" ]]; then
            ...
            while IFS= read -r fn; do
                ...
                'NF >= 2 && $1 !~ /^#/ && index(tolower($1), n) == 1 {print $1}' \
                "$MAPFILE")
            if [[ ${#fuzzy_nicks[@]} -eq 1 ]]; then
                NICK="${fuzzy_nicks[0]}"; TARGET=$(_lookup_target "$NICK")
            elif [[ ${#fuzzy_nicks[@]} -gt 1 ]]; then
                printf "Ambiguous '%s' — did you mean:\n" "$NICK"
                printf '  %s\n' "${fuzzy_nicks[@]}"; exit 1
            else
                printf "Nickname not found: %s\n" "$NICK"; exit 1
            fi
    ...
    *)
        ...
        # SSH: nick [extra ssh args]
            ...
            # Fuzzy fallback: prefix match — "s fm8" connects to fm85 if unambiguous
            if [[ -z "$TARGET" ]]; then
                fuzzy_nicks=()
                while IFS= read -r fn; do
                    [[ -n "$fn" ]] && fuzzy_nicks+=("$fn")
